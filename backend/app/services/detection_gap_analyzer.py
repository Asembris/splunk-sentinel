...
from app.tools.splunk_tools import get_splunk_client, get_splunk_service
...
def _is_sentinel_exact_match(search: dict, technique_id: str) -> bool:
    """
    Return True when this saved search was deployed by Sentinel for exactly
    this technique ID.

    Matches:
      - Name starts with "Sentinel" and contains the bare technique ID
        (e.g. "Sentinel — T1547 Detection").
      - Description contains "Technique: {technique_id}" as written by
        deploy_detection (e.g. "Technique: T1547. Deploy date: …").

    Never cross-matches: T1547 does NOT satisfy T1547.001, and T1547.001
    does NOT satisfy T1547. The negative lookahead (?!\.[0-9]) prevents
    a technique ID from matching when immediately followed by a sub-ID
    separator and digit.
    """
    name = search.get("name", "")
    description = search.get("description", "") or ""
    exact_re = re.escape(technique_id) + r"(?!\.[0-9])"
    if name.startswith("Sentinel") and re.search(exact_re, name):
        return True
    if re.search(r"Technique:\s*" + exact_re, description):
        return True
    return False
...
def _check_technique_coverage(
    ...
    Sentinel-deployed exact-match searches are checked first and always
    yield HIGH confidence, even for techniques not in TECHNIQUE_KEYWORDS.
    Falls through to keyword matching for all other searches.
    """
    # Priority: Sentinel-deployed exact-match (name or description)
    sentinel_matches = [
        s["name"]
        for s in saved_searches
        if _is_sentinel_exact_match(s, technique_id)
    ]
    if sentinel_matches:
        return True, sentinel_matches, "HIGH"

    # Keyword-based matching for non-Sentinel searches
    config = TECHNIQUE_KEYWORDS.get(technique_id)
    if not config:
...
def _is_session_error(exc: Exception) -> bool:
    """Return True for Splunk SDK auth/session errors that warrant a reconnect."""
    msg = str(exc).lower()
    return (
...
async def deploy_detection(
    ...
    Retries once on Splunk session/auth errors after reconnecting the SDK.
    ...
    for attempt in range(2):
        try:
            # Risk 4: Check for duplicate; session errors propagate to outer handler
            try:
                existing_names = [
                    s.name for s in splunk_service.saved_searches
                ]
            except Exception as check_exc:
                if _is_session_error(check_exc):
                    raise
                logger.warning(
            ...
            await loop.run_in_executor(
                None,
                lambda: _svc.saved_searches.create(
                    name,
                    spl,
            ...
            if attempt == 0 and _is_session_error(e):
                logger.warning(
                    "[GAPS] Session error on deploy attempt 1, reconnecting: %s",
                    e,
                )
                client = get_splunk_client()
                await loop.run_in_executor(None, client._reconnect_service)
